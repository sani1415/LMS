...
from .models import Book, Member, Category, Publisher, IssueHistory, LibraryLog
...
def get_issue_history():
    try:
        ...
        if book_name:
            query = query.join(Book).filter(Book.book_name.ilike(f'%{book_name}%'))
        if member_name:
            query = query.join(Member).filter(Member.name.ilike(f'%{member_name}%'))
        if status:
            query = query.filter(IssueHistory.status == status)
...
def add_member(current_user):
    try:
        ...
        db.session.rollback()
        return jsonify({'error': 'Member with this name already exists'}), 409
    except Exception as e:
        db.session.rollback()
        return jsonify({'error': str(e)}), 500
...
def add_publisher(current_user):
    try:
        ...
        db.session.rollback()
        return jsonify({'error': 'Publisher with this name already exists'}), 409
    except Exception as e:
        db.session.rollback()
        return jsonify({'error': str(e)}), 500
...
def get_library_log():
    try:
        ...
        # Build query with filters
        query = LibraryLog.query
        
        if log_type:
            query = query.filter(LibraryLog.log_type == log_type)
        
        # Order by timestamp (newest first)
        query = query.order_by(LibraryLog.timestamp.desc())
...
@token_required
def add_log_entry_api(current_user):
    try:
        data = request.get_json()
